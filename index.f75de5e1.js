...
for (const th of tableHeaders)th.addEventListener("click", (e)=>{
    ...
    for (const header of tableHeaders)header.removeAttribute("data-order");
    ...
    rows.sort((rowA, rowB)=>{
        ...
        let comparison;
        if (isNumberColumn) comparison = cellA - cellB;
...
button.addEventListener("click", (e)=>{
    ...
    for (const input of inputs){
        if (input.name === "name" && input.value.trim().length < 4) {
            pushNotification(150, 100, "Error", "Name must have at least 4 letters", "error");
            return;
        }
        if (input.name === "name" && /\d/.test(input.value.trim())) {
            pushNotification(150, 100, "Error", "Name must not contain numbers", "error");
            return;
        }
        if (input.name === "age") {
            const age = Number(input.value);
            if (age < 18 || age > 90) {
                pushNotification(150, 100, "Error", "Age must be between 18 and 90", "error");
                return;
            }
        }
    }
    ...
    tbody.append(newRow);
    pushNotification(150, 100, "Success", "Employee has been added to the table", "success");
    form.reset();
